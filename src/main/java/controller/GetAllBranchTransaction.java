...
@WebServlet("/home/GetAllBranchTransaction")
public class GetAllBranchTransaction extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(role.equals("MANAGER")) {
				ps = DatabaseConnection.con.prepareStatement("select r.* from Receipt r, Account a where (a.account_no = r.from_account_no and a.branch_id = ?) order by date desc");
				ps.setString(1, branchId);
			}
			else {
				ps = DatabaseConnection.con.prepareStatement("select * from Receipt order by date desc");
			}
